# Установка системы на ПК, загрузчика и правка fstab

Дистрибутив скопирован, после чего необходимо приступить к его настройке.

Для начала загрузитесь в дистрибутив (посредством смены корня):
...
Вы окажетесь в скопированной Calmira GNU/Linux.
...
Замените `/dev/sdX` из первой строки `fstab` на метку корневого раздела, на который устанавливалась система. Например, `/dev/sda3`.
